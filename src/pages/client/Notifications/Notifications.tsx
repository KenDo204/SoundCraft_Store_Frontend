              ...
              {list.map((item) => {
                ...
                return (
                  <div 
                    ...
                    className={`p-6 flex gap-5 transition-all cursor-pointer relative group ${!item.isRead ? 'bg-orange-50/10' : 'hover:bg-stone-50 opacity-80'}`}
                  >
                    {!item.isRead && <div className="absolute left-0 top-0 bottom-0 w-1 bg-orange-600 shadow-[0_0_10px_rgba(234,88,12,0.5)]" />}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-1">
                        <div className="flex items-center gap-2">
                           <span className={`font-bold text-lg leading-tight line-clamp-1 ${!item.isRead ? 'text-stone-900' : 'text-stone-500'}`}>
                             {item.title}
                           </span>
                           <Chip 
                             label={config.label} 
                      ...
                      <p className={`text-sm leading-relaxed mb-1 ${!item.isRead ? 'text-stone-700 font-medium' : 'text-stone-500'}`}>
                        {item.message}
                      </p>
                    </div>
                  </div>
